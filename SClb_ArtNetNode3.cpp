/*
 Name:		SClb_ArtNetNode3.cpp
 Created:	08-Mar-19 20:02:03
 Author:	statuscue
...
uint8_t SCArtNet3::_reply_buffer[ARTNET_REPLY_SIZE];

SCArtNet3::SCArtNet3(IPAddress address)
{
	initialize(0);
	setLocalIP(address);
...
SCArtNet3::SCArtNet3(IPAddress address, IPAddress subnet_mask)
{
	initialize(0);
	setLocalIP(address, subnet_mask);
}

SCArtNet3::SCArtNet3(IPAddress address, IPAddress subnet_mask, uint8_t* buffer)
{
	initialize(buffer);
	setLocalIP(address, subnet_mask);
...
void  SCArtNet3::initialize(uint8_t* b) {
	if (b == 0) {
		// create buffer
...
uint8_t  SCArtNet3::universe(void) {
	return _universe;
}

void SCArtNet3::setUniverse(uint8_t u) {
	_universe = u;
}

void SCArtNet3::setSubnetUniverse(uint8_t s, uint8_t u) {
	_universe = ((s & 0x0f) << 4) | (u & 0x0f);
}

void SCArtNet3::setUniverseAddress(uint8_t u) {
	if (u != 0x7f) {
		if (u & 0x80) {
...
void SCArtNet3::setSubnetAddress(uint8_t u) {
	if (u != 0x7f) {
		if (u & 0x80) {
...
void SCArtNet3::setNetAddress(uint8_t s) {
	if (s & 0x80) {
		_net = (s & 0x7F);
	}
}

void SCArtNet3::setLocalIP(IPAddress a) {
	_my_address = a;
	// Set IP Node
...
void SCArtNet3::setLocalIP(IPAddress a, IPAddress sn) {
	setLocalIP(a);
...
int  SCArtNet3::numberOfSlots(void) {
	return _dmx_slots;
}

void SCArtNet3::setNumberOfSlots(int n) {
	_dmx_slots = n;
}

uint8_t SCArtNet3::getSlot(int slot) {
	return _packet_buffer[ARTNET_ADDRESS_OFFSET + slot];
}

uint8_t SCArtNet3::getHTPSlot(int slot) {
	return _dmx_buffer_c[slot - 1];
}

void SCArtNet3::setSlot(int slot, uint8_t value) {
	_packet_buffer[ARTNET_ADDRESS_OFFSET + slot] = value;
}

uint8_t* SCArtNet3::dmxData(void) {
	return &_packet_buffer[ARTNET_ADDRESS_OFFSET + 1];
}

uint8_t* SCArtNet3::replyData(void) {
	return _reply_buffer;
}

char* SCArtNet3::shortName(void) {
	return (char*)&_reply_buffer[26];
}

char* SCArtNet3::longName(void) {
	return (char*)&_reply_buffer[44];
}

void SCArtNet3::setNodeName(const char* name) {
	//strcpy((char*)&_reply_buffer[26], name);
	strcpy(longName(), name);
}

uint8_t SCArtNet3::readDMXPacket(UDP* eUDP) {
...
uint8_t SCArtNet3::readDMXPacketContents(UDP* eUDP, int packetSize) {
	if (packetSize > 0) {
...
*/
uint16_t SCArtNet3::readArtNetPacket(UDP* eUDP) {
...
uint16_t SCArtNet3::readArtNetPacketContents(UDP* eUDP, int packetSize) {
	if (!_using_htp) {
		_dmx_slots = 0;
...
uint16_t SCArtNet3::readArtDMX(UDP* eUDP, uint16_t slots, int packetSize) {
	uint16_t opcode = ARTNET_NOP;
	if (_using_htp) {
...
void SCArtNet3::sendDMX(UDP* eUDP, IPAddress to_ip) {
...
*/
void SCArtNet3::send_art_poll_reply(UDP* eUDP) {
...
void SCArtNet3::send_art_tod(UDP* wUDP, uint8_t* todata, uint8_t ucount) {
...
void SCArtNet3::send_art_rdm(UDP* wUDP, uint8_t* rdmdata, IPAddress toa) {
...
void SCArtNet3::setArtTodRequestCallback(ArtNetDataRecvCallback callback) {
	_art_tod_req_callback = callback;
}

void SCArtNet3::setArtRDMCallback(ArtNetDataRecvCallback callback) {
	_art_rdm_callback = callback;
}

void SCArtNet3::setArtCommandCallback(ArtNetDataRecvCallback callback) {
	_art_cmd_callback = callback;
}

uint16_t SCArtNet3::parse_header(void) {
...
*/
uint16_t SCArtNet3::parse_art_address(UDP* wUDP) {
...
uint16_t SCArtNet3::parse_art_tod_request(UDP* wUDP) {
...
uint16_t SCArtNet3::parse_art_tod_control(UDP* wUDP) {
	if (_art_tod_req_callback != NULL) {
		if (_packet_buffer[21] == _net) {
...
uint16_t SCArtNet3::parse_art_rdm(UDP* wUDP) {
...
void SCArtNet3::parse_art_cmd(UDP* wUDP) {
...
void  SCArtNet3::initializePollReply(void) {
	int i;
	...
	strcpy((char*)&_reply_buffer[26], "SCArtNetDMX3");
	strcpy((char*)&_reply_buffer[44], "SCArtNetDMX3");
	...
	_reply_buffer[211] = 0x3;							//  BindIndex
